...
const LINK_STYLE = 'color: var(--text-color, #fff); text-decoration: underline;';
...
function updateHtml() {
    ...
    for (const [listId, folderName] of Object.entries(SECTIONS)) {
        ...
        let listHtml = '';
        if (pdfs.length === 0) {
            listHtml = '    <li><span style="color: #888;">Ez dago fitxategirik.</span></li>';
        } else {
        ...
        const regex = new RegExp(`(<ul[^>]*id="${listId}"[^>]*>)[\\s\\S]*?(<\\/ul>)`, 'i');
        
        if (regex.test(htmlContent)) {
            htmlContent = htmlContent.replace(regex, `$1\n${listHtml}$2`);
            changesMade = true;
            console.log(`✅ Sección '${listId}' actualizada con ${pdfs.length} archivos.`);
        } else {
            console.warn(`⚠️ No se encontró el elemento con id="${listId}" en el HTML.`);
        }
    }
